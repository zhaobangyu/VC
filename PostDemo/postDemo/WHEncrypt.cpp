...
bool CWHEncrypt::MD5Encrypt(LPCTSTR pszSourceData, TCHAR szMD5Result[LEN_MD5])
{
	...
	CT2CA strResultData(szMD5Result);
	...
	for (INT i = 0; i < 16; i++)
	{
		_stprintf(&szMD5Result[i * 2],TEXT("%02X"), szResult[i]);
	}
...
bool CWHEncrypt::XorEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncrypData, WORD wMaxCount)
{
	...
	for (WORD i=0;i<wTimes;i++)
	{
		...
		swprintf_s(szEncrypData + i * 8,sizeof(szEncrypData), L"%04X%04X", wRandKey[i%ENCRYPT_KEY_LEN], wTempCode);
	}
